...
export default class SearchForm extends Component {
    ...
    handleSubmit(event) {
        ...
        if (this.state.subject == '' && this.state.course == '' && this.state.professor == '') {
            fetch('https://hackathon-singularity-backend.herokuapp.com/grades/' + this.state.semester + '/' + this.state.year)
                .then(res => res.json())
                .then(json => {
                    this.setState({ returnedJSON: JSON.stringify(json) });
                    for (var i = 0; i < json.length; i++) {

                        fetch('https://hackathon-singularity-backend.herokuapp.com/rating/' + json[i]["prof"])
                        .then(res2 => res2.json())
                        .then(json2 => {
                            try {
            ...
            fetch('https://hackathon-singularity-backend.herokuapp.com/grades/' + this.state.semester + '/' + this.state.year + '/' + this.state.professor)
                .then(res => res.json())
                .then(json => {
                    this.setState({ returnedJSON: JSON.stringify(json) });
                    for (var i = 0; i < json.length; i++) {
            ...
            fetch('https://hackathon-singularity-backend.herokuapp.com/grades/' + this.state.semester + '/' + this.state.year + '/' + this.state.subject + '/' + this.state.course)
                .then(res => res.json())
                .then(json => {
                    this.setState({ returnedJSON: JSON.stringify(json) });
                    for (var i = 0; i < json.length; i++) {
            ...
            fetch('https://hackathon-singularity-backend.herokuapp.com/grades/best/' + this.state.semester + '/' + this.state.year + '/' + this.state.subject + '/' + this.state.course)
                .then(res => res.json())
                .then(json => {
                    this.setState({ returnedJSON: JSON.stringify(json) });
                    for (var i = 0; i < json.length; i++) {
